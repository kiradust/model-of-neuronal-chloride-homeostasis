...
def f5a():
    dez=plm(dz=1e-6,two=1,xt=120,tt=360)
...
def f5b(moldelt=0):
    XF=[-1.1,-1.05,-1.0,-0.95,-0.9,-0.85,-0.8,-0.75,-0.7,-0.65,-0.6,-0.55,-0.5,-0.45,-0.4]
    XFp=[]
    cl=[]
    vm=[]
    k=[]
    ...
    plt.figure()
    gs = gridspec.GridSpec(4,1,height_ratios=[1.5,0.33,0.33,0.33])
    a0=plt.subplot(gs[0])
    ...
    a1.plot(z,w,'ko')
    a1.set_ylim([7.5e-14,12e-14])
    a2=plt.subplot(gs[2])
    a2.plot(zi,newx,color='m') #conc X
    a2.plot(z,x,'mo')
    a2.set_ylim([0.13,0.22])
    a3=plt.subplot(gs[3])
    a3.plot(z,df)
    a3.set_ylim([0,10])
