...
def delta_gs3(Gk=[70],Gna=[20],Gkcc=[20],Gcl=[20],molinit=0,Gextra=[0]):
    ...
    for i in range(max(len(Gcl),len(Gkcc),len(Gk),len(Gna),len(Gextra))):
        ...
        for l in prange:
            ...
            if (gk+gextra)*(gcl+gextra)+gkcc*gcl+gk*gkcc+2*gkcc*gextra !=0:
...
def gextrasweep(gextraa=range(0,100)):
# RUN THIS FOR GEXTRA SWEEP SIM
    print("g_extra sweep")
    dga = delta_gs3(Gcl=[20],Gk=[70],Gna=[20],Gkcc=[20],Gextra=gextraa)
    minifig(dga,x=np.log10(25),yl=[[-100,-50],[1.95e-12,2.10e-12]])
    plt.savefig('ana.svg',dpi=150)
    plt.show()

    plt.plot(dga[3])
    plt.show()

    minithreefig([dga[0],dga[1],dga[2],dga[5],dga[6],np.array(dga[5])-np.array(dga[1]),dga[3]],'r',x=np.log10(25),yl=[[40,70],[1.9e-12,2.10e-12],[0,20]])
    plt.savefig('ana3.svg',dpi=150)
    plt.show()
    return dga
