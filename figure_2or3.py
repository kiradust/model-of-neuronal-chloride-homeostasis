...
def f3d(new=0,title='f3d.eps'): #doubles as f6f when new=1
    ...
    if new==0:
        ...
        plt.figure()
        plt.plot(P,np.subtract(gp[5],gp[3]),color='k')
        plt.plot(P,np.subtract(gp_nokcc[5],gp_nokcc[3]),linestyle='--',color='k')
    plt.savefig(title)
    plt.show()
    return
